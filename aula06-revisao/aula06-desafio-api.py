...
for i in range (len(endpoints)):
    nome_endpoints = endpoints[i]
    reqs_endpoints = status[i]

    sucesso, erro, percentual, classificacao = analisar_endpoint(reqs_endpoints)

    print(f"Endpoint: {nome_endpoints}")
    print(f"Requisições: {reqs_endpoints}")
    ...
    if erro > qtd_maior_erro:
        qtd_maior_erro = erro
        endpoint_maior_erro = nome_endpoints
